module.exports = function(app, apiRoutes, io){
					...
					if(!err){
						var result = rs.filter(function(payment){
							if(payment._credit && payment._credit.data){
									return (payment._credit.data.status == 'Pagado' && !payment.data.paymentDone && !payment.data.invalid_payment);
							}
						});
		...
		function update(req, res){
			...
			Model.update( { _id : mongoose.Types.ObjectId(req.params.id)} , REQ , function(err, rs){
				if(rs){
					res.status(200).json(rs);
				}else{
					res.status(500).json(err)
