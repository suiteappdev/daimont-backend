module.exports = function(app, apiRoutes, io){
		...
		function getCurrent(req, res){
			var REQ = req.params; 
			try{
				Model.findOne({ "_user" : mongoose.Types.ObjectId(req.headers['x-daimont-user']), "createdAt": -1 }).populate("_user").populate("_payment").populate("_contract").exec(function(err, rs){
					if(!err){
						res.status(200).json(rs || []);
					}else{
						res.status(500).json(err);
