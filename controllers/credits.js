		...
		function remove(req, res){
 		...
 		function morosos(req, res){
			...
			try{
				...
				Model.find({"data.status" : 'Consignado', "data.deposited_time_server" : { $lte: cutoffDate}, "data.deposited_time" : $or [{ $lte: cutoffDate}]}).sort("-createdAt").populate("_user").populate("_payment").populate("_contract").populate("_approvedby").exec(function(err, rs){
					if(!err){
						res.status(200).json(rs || []);
					}else{
						res.status(500).json(err);
