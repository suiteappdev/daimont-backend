		...
		function remove(req, res){
 		...
 		function morosos(req, res){
			...
			try{
				...
				Model.find({"data.status" : 'Consignado', $and : [ { "data.deposited_time_server" : { $lte: cutoffDate} }, { "data.deposited_time" : { $lte: cutoffDate} } ]}).sort("-createdAt").populate("_user").populate("_payment").populate("_contract").populate("_approvedby").exec(function(err, rs){
					if(!err){
						res.status(200).json(rs || []);
					}else{
						res.status(500).json(err);
