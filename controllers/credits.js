module.exports = function(app, apiRoutes, io){
		...
		function remove(req, res){
 		...
 		function pendiente(req, res){
			var REQ = req.params; 
			try{
				Model.find({"data.hidden" : false, "data.status" : 'Pendiente', "width_offer" : false}).sort("-createdAt").populate("_user").populate("_payment").populate("_contract").exec(function(err, rs){
					if(!err){
						res.status(200).json(rs || []);
					}else{
						res.status(500).json(err);
